...
void master_main(int server_fd, shared_memory_t * shm, semaphores_t * sems, int ipc_socket, config_t * conf){
    ...
    while(1){
        ...
        if (client_fd < 0) {
            // Se o sistema estiver sem FDs, pausa breve (10ms) para recuperar
        ...
        // 1. Verificar Semáforos (Backpressure Lógico)
        ...
        if (status == 0) {
            ...
            shm->queue.socket_fds[shm->queue.rear] = client_fd; // Apenas para debug/tracking
            ...
            // FALHA: Buffer cheio (-2) ou erro. Devolver slot do semáforo.
        ...
        // Master já não precisa deste FD
        close(client_fd);
    }
}